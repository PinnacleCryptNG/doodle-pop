...
const FALLBACK_DATA_FILE = path.join(process.cwd(), ".local_notes_db.json");
...
async function startServer() {
  ...
  app.post("/api/auth/signup", async (req: Request, res: Response) => {
    try {
      ...
      // We do NOT set session cookie or auto-login until email is confirmed
      return res.status(201).json({
        success: true,
        requiresVerification: true,
        email: cleanEmail,
  ...
  // Helper to create user-scoped Supabase client that enforces RLS
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        if (error) {
          ...
          // If table doesn't exist yet, fall back seamlessly
        } else if (data) {
          return res.json({ notes: data });
        }
      }

      // Fallback local DB (strictly isolated by user_id)
  ...
  // 3. UPDATE A NOTE (Strict check that note belongs to user)
  app.put("/api/notes/:id", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      // Strict user security validation: A user can NEVER touch another user's notes
  ...
  // 4. DELETE A NOTE (Strict check that note belongs to user)
  app.delete("/api/notes/:id", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const existing = fallbackDb.notes[id];
      if (!existing) {
        // Return 200/success for idempotent deletion
        return res.json({ success: true, id, message: "Note already deleted" });
      }

      // Strict user security validation
      if (existing.user_id !== userId) {
        return res.status(403).json({ error: "Access denied: You do not own this note" });
      }
  ...
  // 5. BATCH SYNC ENDPOINT for Offline Sync Engine
  // Synchronizes locally created/updated notes and offline deletions
  app.post("/api/notes/sync", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      // Process client upserts (with conflict resolution: newest updated_at wins)
      for (const clientNote of clientNotes) {
        // Ensure user_id cannot be spoofed by client
      ...
      // Return refreshed full list of notes for this user
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Full-Stack Notes Server running on http://0.0.0.0:${PORT}`);
  });
}
